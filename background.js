...
//   if (
//     !localStorage.lastAnalyticTrackingTime ||
//     now - localStorage.lastAnalyticTrackingTime >= analyticsTimeInterval
//   ) {
...
//   }
//   var timeLeft =
//     analyticsTimeInterval - (now - localStorage.lastAnalyticTrackingTime);
...
function drawArrow(ctx, color) {
  ...
  var center = ctx.canvas.width / 2.3; // Don't know how it got off center, but adding 0.3 helps
  ctx.beginPath();
  ctx.lineJoin = "round";
  ctx.strokeStyle = color;
  ctx.lineCap = "round";
  ctx.lineWidth = Math.round(w * 0.04);
  ctx.moveTo(center + 0.0625 * w, center - 0.325 * w);
  ctx.lineTo(center + 0.0625 * w, center + 0.2125 * w);
  ctx.moveTo(center - 0.1 * w, center + 0.0375 * w);
  ctx.lineTo(center + 0.0625 * w, center + 0.2125 * w);
  ctx.lineTo(center + 0.225 * w, center + 0.0375 * w);
  ctx.moveTo(center - 0.325 * w, center + 0.25 * w);
  ctx.lineTo(center - 0.325 * w, center + 0.4125 * w);
  ctx.lineTo(center + 0.45 * w, center + 0.4125 * w);
  ctx.lineTo(center + 0.45 * w, center + 0.25 * w);
  ctx.stroke();
}
...
function drawCompleteBadge(ctx) {
  ...
  var center = ctx.canvas.width / 2.25;
  ctx.beginPath();
  ...
  chrome.downloads.search({}, function(items) {
    ...
    items.forEach(function(item) {
      if (item.state == "in_progress") {
        ...
        setTimeLeft(
          new Date(item.estimatedEndTime).getTime() - new Date().getTime()
        );
        if (item.totalBytes) {
        ...
        var dangerous = item.danger != "safe" && item.danger != "accepted";
    ...
    if (anyInProgress && pollProgress.tid < 0) {
      pollProgress.start();
    }
  });
}
...
function showNotification(item) {
  item.basename = item.filename.substring(
    Math.max(item.filename.lastIndexOf("\\"), item.filename.lastIndexOf("/")) +
      1
  );
  if (item.basename.length > 50) {
    item.basename =
      item.basename.substr(0, 40) +
      "..." +
